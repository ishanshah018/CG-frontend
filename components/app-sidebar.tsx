...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="relative bg-zinc-900 **:font-heading [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] after:absolute after:top-0 after:right-0 after:w-px after:h-full after:bg-zinc-900 after:z-50" style={{ backgroundColor: '#101213' }}>
      <SidebarHeader className="border-b border-white/10 p-3 md:p-4 group-data-[state=collapsed]:p-2" style={{ backgroundColor: '#101213' }}>
        <div className="flex items-center gap-2 group-data-[state=collapsed]:justify-center">
          <img
            src="/logo.png"
            alt="CertGen Logo"
            className="h-8 w-8 md:h-10 md:w-10 group-data-[state=collapsed]:h-8 group-data-[state=collapsed]:w-8 rounded-lg object-cover shrink-0"
          />
          <div className="group-data-[state=collapsed]:hidden">
            <h2 className="text-lg md:text-xl font-bold text-white">
              Certify<span className="text-[#3ab5e2]">Gen</span>
            </h2>
          </div>
        </div>
      </SidebarHeader>
              ...
              {filteredMenuItems.map((item) => {
                  ...
                  return (
                    <SidebarMenuItem key={item.title}>
                      ...
                      >
                        ...
                        <ChevronRight
                          className={`ml-auto h-3 w-3 md:h-4 md:w-4 transition-transform ${isSubmenuOpen ? "rotate-90" : ""
                            }`}
                        />
                      </SidebarMenuButton>
                      <div
                          ...
                          {item.items?.map((subItem) => {
                            ...
                            return (
                              <SidebarMenuSubItem key={subItem.title}>
                                <SidebarMenuSubButton
                                  asChild
                                  className={`h-12.5 text-[17px] font-medium tracking-[0.3px] leading-none text-white/70 hover:scale-[1.01] transition-transform rounded-2xl ${isSubActive
                ...
                return (
                  <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton
                      asChild
                      className={`h-12.5 text-[17px] font-medium tracking-[0.3px] leading-none text-white/80 hover:scale-[1.01] transition-transform rounded-2xl ${isActive ? "bg-brand-primary font-semibold" : ""
                        }`}
